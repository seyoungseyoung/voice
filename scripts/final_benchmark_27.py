"""
최종 LLM 벤치마크 - exam.md 전체 27개 케이스
Category A: 전통적 보이스피싱 (10개)
Category B: 정상 케이스 (3개)
...
def main():
    ...
    available = {n: c for n, c in clients.items() if c.is_available()}

    if not available:
        print("❌ 사용 가능한 LLM이 없습니다.")
        return

    print(f"✅ 테스트 LLM: {', '.join(available.keys())}\n")
    ...
    for test_case in test_cases:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(test_llm, c, test_case): n for n, c in available.items()}
    ...
    categories = {"A": "전통적 보이스피싱", "B": "정상 케이스", "C": "URL 피싱", "D": "신규 유형"}

    for cat_id, cat_name in categories.items():
        print(f"[{cat_id}] {cat_name}")
        print("-"*60)

        cat_cases = [tc for tc in test_cases if tc["cat"] == cat_id]

        for llm_name in available.keys():
            model_name = available[llm_name].model_name
            correct = 0
            total = 0

            for tc in cat_cases:
                result = next((r for r in all_results if r["id"] == tc["id"] and r["model"] == model_name), None)
                if result:
                    total += 1
                    if tc["type"] == "legitimate":
                        if result["score"] <= tc["max"]:
                            correct += 1
                    else:
                        if result["score"] >= tc["min"]:
                            correct += 1

            acc = (correct / total * 100) if total > 0 else 0
            print(f"  {llm_name:<15} {acc:>5.1f}% ({correct}/{total})")
    ...
    for llm_name in sorted(available.keys()):
        model_name = available[llm_name].model_name
        correct = 0
        total = 0
