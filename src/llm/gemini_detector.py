...
- 가족/지인을 사칭하며 + 아래 모두 해당:
  - 긴급한 금전 요구 (사고, 납치, 급전 필요)
  - 즉각적인 송금/이체 요청 (백만원 이상 고액)
  - 비정상적 상황 설정 (전화번호 변경, 문자로만 연락)
  - **중요**: 실제 통화 중 소액(10만원 이하) 요청은 정상일 수 있음
